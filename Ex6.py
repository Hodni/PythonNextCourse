...
# Function to draw a continuous line between points
...
image = Image.new("RGBA", (450, 450), (0, 0, 0, 0))

# Drawing connected lines between the points in the lists first and second
draw_lines(image, first)
draw_lines(image, second)

# Open the background image
background = Image.open("Holon.jpg")
bg_width, bg_height = background.size

# Create a new image to paste the lines on the background
new_image = Image.new("RGBA", (bg_width, bg_height), (0, 0, 0, 0))

# Paste the lines image onto the background image multiple times
for x in range(0, bg_width, 400):
    for y in range(0, bg_height, 400):
        new_image.paste(image, (x, y), image)

# Paste the combined lines image onto the background image
combined_image = Image.alpha_composite(background.convert("RGBA"), new_image)

# Save the final image
combined_image.save("Holon_with_bold_lines.png")
